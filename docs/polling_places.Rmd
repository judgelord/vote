...
## TL;DR: Download VAN-formatted elections.wi.gov + Google Civic polling locations with the link to the left.
...
- Several names have spaces in them 
...
van %<>% mutate_if(is.character, str_to_upper)
...
van_new <- locations %>% 
  mutate(Address = str_remove(PollingPlaceAddress, ", .*") %>% 
         ...
         # Muni is not the same as City
          Muni = Muni %>% str_remove("(TOWN|CITY|VILLAGE) OF "), 
...
# Discrepencies between WEC and VAN data
...
### Discrepencies in Polling Location Names

> We are not matching on polling location name
...
van %>%  filter(!Address %in% upload$Address) %>% 
  select(County,  Address, `Polling Location`) %>% 
  distinct() %>% 
  kablebox()
```
...
(Location Name Changes, New Addresses)
...
Download VAN-formatted combined elections.wi.gov + Google Civic polling locations [here](https://github.com/judgelord/vote/tree/master/data) (click on "raw" and then save) or go to this link and then save: 

# Upload to VAN: `r paste0("https://raw.githubusercontent.com/judgelord/vote/master/data/", upload_file) %>% str_replace_all(" ", "%20")`
...
>Precinct ID, Precinct Code and Precinct Name are ones that should always be the same from VAN. If state's name is slightly different for a polling location, if it's easier to just change it to that, that is fine. If polling location is described differently, just replace to what state has. I think that answers your question, but let me know if it doesn't.
...
>This is something that get's applied in bulk after I upload polling locations. So if it's easier for you to just not deal with those that's fine. If it isn't than yeah Google Civic can be the authenticating authority and DPW-November Election as contact info
...
1. The Addresses in the VAN data capitalize oddly--"1St" "2Nd" "3Rd" etc.--I made the other addresses match this format, but it is odd and will likely make trouble in other merges. All caps is easier.
