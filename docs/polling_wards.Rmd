...
added <- upload %>% 
  filter(Address_WEC != Address_VAN) %>% 
  select(County, `Precinct Name`,Address_WEC, Address_VAN, `Polling Location`, `Date Authenticated*`) %>% 
  distinct() 
...
poll_address_changes <- full_join(added, deleted)  

VANmissing <- read_csv(here::here("data", "VAN_missing_precincts.csv"))

poll_address_changes %<>% anti_join(VANmissing %>% select(`Precinct Name`, `Address`))


poll_address_changes %<>%
  separate(`Precinct Name`, into = c("county", "Muni", "ward"), sep = " - ") %>% 
  select(-county) %>% 
