...
ggplot( aes(x = long, y = lat, group = group, fill = factor(changes)) ) +
...
The geodata has a few errors, but these points are mostly correct.
...
wi %>% 
  ...
  mutate(center_lat = mean(range(lat) ),
         center_long = mean(range(long) ) ) %>%
ggplot( aes(x = long, y = lat, group = group, fill = factor(net)) ) +
  geom_polygon(color = "white") +
  geom_point(aes(x = Longitude.Primary, 
                 y = Latitude.Primary), 
             color = "Red", alpha = .5) +
